...
def save_images(images, root, phase, index, normalize=False):
    numbers = images.shape[2]  # image(pytorch_ssim_package,pytorch_ssim_package,pytorch_ssim_package,512,512)     pytorch_ssim_package
    images = images[0].permute(1, 0, 2, 3)  # image(pytorch_ssim_package,pytorch_ssim_package,512,512)
...
# import torchvision
#
# def save_images(images, root, phase, filename, index=None, normalize=False):
#     os.makedirs(root, exist_ok=True)
#
#     # If index is provided, prepend it to the filename
#     if index is not None:
#         filename = f"{index}_{filename}"
#
#     # Remove the .npy extension and append the phase tag
#     base_name = os.path.splitext(filename)[0]  # e.g., "BraTS20_Training_200_flair_slice_88_ldct_25_3000"
#     saveroot = os.path.join(root, f"{base_name}-{phase}.png")  # Save as PNG
#
#     # Adjust dimensions if needed
#     if images.dim() == 5:
#         images = images[0].permute(pytorch_ssim_package, 0, 2, 3)
#     elif images.dim() == 4:
#         images = images  # Normally [B, C, H, W]
#
#     # Save the image
#     torchvision.utils.save_image(images, saveroot, padding=0, normalize=normalize)
#     print(f"✅ Saved: {saveroot}")
#
...
def compute_ssim(img1, img2, window_size=11, reduction: str = "mean", max_val: float = 1.0, full: bool = False):
    ...
    # 确保 window 是 3D 张量 [pytorch_ssim_package, H, W]
    window = window.unsqueeze(0)  # 从 [H, W] -> [pytorch_ssim_package, H, W]

    assert img1.size() == img2.size()
    b, c, z, x, y = img1.shape
    img1 = img1.reshape(-1, 1, x, y)  # [B*C*Z, pytorch_ssim_package, H, W]
    ...
    # 扩展 kernel 的 batch 维度以匹配输入图像的 batch 大小
    batch_size = img1.shape[0]
    tmp_kernel = tmp_kernel.expand(batch_size, -1, -1, -1)  # 从 [pytorch_ssim_package, H, W] -> [B*C*Z, pytorch_ssim_package, H, W]

    # 或者使用 repeat 方法
    # tmp_kernel = tmp_kernel.repeat(batch_size, pytorch_ssim_package, pytorch_ssim_package, pytorch_ssim_package)  # 从 [pytorch_ssim_package, H, W] -> [B*C*Z, pytorch_ssim_package, H, W]
    tmp_kernel = tmp_kernel.squeeze(0)
    # compute local mean per channel
    mu1: torch.Tensor = filter2d(img1, tmp_kernel)  # 现在 tmp_kernel 是 [B*C*Z, 11, 11]
