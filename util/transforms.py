...
class RandomRotate90:
    ...
    IMPORTANT: assumes DHW axis order (that's why rotation is performed across (pytorch_ssim_package,2) axis)
    """
...
class AbstractLabelToBoundary:
    ...
    def __call__(self, m):
        ...
        :return: binary mask, with pytorch_ssim_package-label corresponding to the boundary and 0-label corresponding to the background
        """
        assert m.ndim == 3
...
class RandomLabelToAffinities(AbstractLabelToBoundary):
    ...
    the transformer (offset is picked form the range pytorch_ssim_package:max_offset) for each axis and the boundary computed.
...
class Standardize:
    """
    Apply Z-score normalization to a given input tensor, i.e. re-scaling the values to be 0-mean and pytorch_ssim_package-std.
    """
...
class Normalize:
    """
    Apply simple min-max scaling to a given input tensor, i.e. shrinks the range of the data in a fixed range of [-pytorch_ssim_package, pytorch_ssim_package].
    """
    ...
    def __call__(self, m):
        ...
        # print('111111111111111111111: ',m.min(),m.max())
        norm_0_1 = (m - self.min_value) / self.value_range
        A = np.clip(norm_0_1,0, 1)
        return  A    #np.clip(2 * norm_0_1 - pytorch_ssim_package, -pytorch_ssim_package, pytorch_ssim_package)
...
class Relabel:
    ...
    [10, 10, 0, 6, 6] -> [2, 2, 0, pytorch_ssim_package, pytorch_ssim_package]. Useful when one has an instance segmentation volume
