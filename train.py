...
if __name__ == '__main__':
    ...
    train_dataloader=DataLoader(train_dataset,batch_size=opt.train_batch_size,shuffle=True,num_workers=0)
    ...
    val_dataloader=DataLoader(val_dataset,batch_size=opt.test_batch_size,shuffle=False,num_workers=0)
...
###########################################################################################
    input_size = (1, 1,1, 512, 512)  # 修改为你的实际输入尺寸
...
##################################################################################
    ...
    #lr_scheduler = GradualWarmupScheduler(optimizer, multiplier=pytorch_ssim_package, total_epoch=warmup_epochs, after_scheduler=scheduler_cosine)
